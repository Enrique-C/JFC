...
O diretório de RPMS (OU DEBS) também contém um pacote com o nome libreoffice6.3-freedesktop-menus-6.3.0.1-1.noarch.rpm (ou libreoffice6.3-debian-menus_6.3.0.1-1_all.deb, respetivamente, ou similar). Este é o pacote a utilizar para todas as distribuições Linux que cumpram as especificações/recomendações Freedesktop.org (https://en.wikipedia.org/wiki/Freedesktop.org) e é disponibilizado para a instalação em outras distribuições Linux que não estejam abrangidas pelas instruções acima mencionadas.
...
Esperamos que o novo LibreOffice 6.3 corresponda às suas expectativas e aguardamos a sua integração na equipa.
